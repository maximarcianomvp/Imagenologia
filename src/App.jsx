...
function CensoView({ solicitudes, loading }) {
  ...
  function exportExcel() {
    ...
    const rows = [];
    rows.push({ "Doctor": "CENSO DE IMAGENOLOGÍA", "Paciente": "", "Estudio": "", "Modalidad": "", "Servicio": "", "Prioridad": "", "Horario": "", "Estado": "", "Hora": fecha });
    rows.push({});
    groupKeys.forEach(doctor => {
      rows.push({ "Doctor": doctor.toUpperCase(), "Paciente": `(${grouped[doctor].length} estudio${grouped[doctor].length !== 1 ? "s" : ""})`, "Estudio": "", "Modalidad": "", "Servicio": "", "Prioridad": "", "Horario": "", "Estado": "", "Hora": "" });
      grouped[doctor].forEach(sol => {
        const svc = sol.servicio_solicitante === "Otro" ? sol.servicio_otro : sol.servicio_solicitante;
        rows.push({ "Doctor": "", "Paciente": sol.nombre_paciente, "Estudio": sol.tipo_estudio, "Modalidad": sol.sub_tipo || "—", "Servicio": svc, "Prioridad": sol.prioridad, "Horario": sol.horario_programado || "—", "Estado": ESTADOS_CFG[sol.estado]?.label || sol.estado, "Hora": new Date(sol.created_at).toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" }) });
      });
      rows.push({});
    });
    rows.push({ "Doctor": "TOTALES", "Paciente": `Total: ${cnt.total}`, "Estudio": `Realizados: ${cnt.realizado}`, "Modalidad": `En proceso: ${cnt.en_proceso}`, "Servicio": `Pendientes: ${cnt.pendiente}`, "Prioridad": "", "Horario": "", "Estado": "", "Hora": "" });
    const ws = XLSX.utils.json_to_sheet(rows);
    ws["!cols"] = [20, 28, 18, 16, 20, 12, 14, 14, 12].map(w => ({ wch: w }));
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead>
                    <tr style={{ background: `${clr.light}` }}>
                      {["Hora", "Paciente", "Estudio", "Servicio", "Prioridad", "Horario prog.", "Estado"].map(h => (
