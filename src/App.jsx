...
function SolicitudModal({ sol, adscritos, token, onUpdate, onClose }) {
  ...
  const [nombrePaciente, setNombrePaciente] = useState(sol.nombre_paciente || "");
  const [edad,     setEdad]    = useState(sol.edad              || "");
  const [sexo,     setSexo]    = useState(sol.sexo              || "");
  const [servicio, setServicio]= useState(sol.servicio_solicitante || "");
  const [servicioOtro, setServicioOtro] = useState(sol.servicio_otro || "");
  const [motivo,   setMotivo]  = useState(sol.motivo_clinico    || "");
  const [tipoEstudio, setTipoEstudio] = useState(sol.tipo_estudio || "");
  const [subTipo,  setSubTipo] = useState(sol.sub_tipo          || "");
  const [protocolo,setProtocolo]=useState(sol.protocolo         || "");
  const [prioridad,setPrioridad]=useState(sol.prioridad         || "Rutina");
  const [nombreMedico, setNombreMedico] = useState(sol.nombre_medico || "");
  const [mipSolicitante, setMipSolicitante] = useState(sol.mip_solicitante || "");
  const [telefono, setTelefono]= useState(sol.telefono          || "");
  const [ubicacion,setUbicacion]=useState(sol.ubicacion_paciente|| "");
  const [creatinina, setCreatinina] = useState(sol.creatinina   || "");
  const [busy, setBusy]        = useState(false);

  async function save() {
    setBusy(true);
    await db.updateSolicitud(sol.id, {
      nss_expediente: exp, adscrito_id: adscrito || null, estado, horario_programado: horario,
      notas, hallazgos, fecha_recordatorio: fechaRecordatorio || null,
      nombre_paciente: nombrePaciente, edad, sexo,
      servicio_solicitante: servicio, servicio_otro: servicio === "Otro" ? servicioOtro : null,
      motivo_clinico: motivo, tipo_estudio: tipoEstudio, sub_tipo: subTipo || null,
      protocolo: protocolo || null, prioridad,
      nombre_medico: nombreMedico, mip_solicitante: mipSolicitante || null,
      telefono, ubicacion_paciente: ubicacion || null, creatinina: creatinina || null
    }, token);
    await onUpdate();
    setBusy(false);
    onClose();
    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 16 }}>
      <div style={{ ...cx.card, width: "100%", maxWidth: 560, maxHeight: "92vh", overflowY: "auto" }}>
        ...
        <div style={{ background: "#F8FAFC", borderRadius: 8, padding: "12px 14px", marginBottom: 12 }}>
          <p style={{ margin: "0 0 10px", fontSize: 11, fontWeight: 700, color: "#475569", letterSpacing: "0.06em" }}>🧑‍🦱 DATOS DEL PACIENTE</p>
          <FG label="Nombre del paciente">
            <input style={cx.input} value={nombrePaciente} onChange={e => setNombrePaciente(e.target.value)} />
          </FG>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <FG label="Edad">
              <input style={cx.input} value={edad} onChange={e => setEdad(e.target.value)} />
            </FG>
            <FG label="Sexo">
              <select style={cx.input} value={sexo} onChange={e => setSexo(e.target.value)}>
                <option value="">—</option>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <FG label="Servicio solicitante">
              <select style={cx.input} value={servicio} onChange={e => setServicio(e.target.value)}>
                {SERVICIOS.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            </FG>
            <FG label="Ubicación del paciente" optional>
              <input style={cx.input} value={ubicacion} onChange={e => setUbicacion(e.target.value)} placeholder="Cama, sala..." />
            </FG>
          </div>
          {servicio === "Otro" && (
            <FG label="Especificar servicio">
              <input style={cx.input} value={servicioOtro} onChange={e => setServicioOtro(e.target.value)} />
            </FG>
          )}
          <FG label="Diagnóstico / Motivo clínico">
            <textarea style={{ ...cx.input, resize: "none" }} rows={2} value={motivo} onChange={e => setMotivo(e.target.value)} />
          </FG>
        </div>

        {/* Estudio — editable */}
        <div style={{ background: "#EFF6FF", borderRadius: 8, padding: "12px 14px", marginBottom: 12 }}>
          <p style={{ margin: "0 0 10px", fontSize: 11, fontWeight: 700, color: "#1E40AF", letterSpacing: "0.06em" }}>🔬 ESTUDIO</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <FG label="Tipo de estudio">
              <input style={cx.input} value={tipoEstudio} onChange={e => setTipoEstudio(e.target.value)} />
            </FG>
            <FG label="Sub-tipo" optional>
              <input style={cx.input} value={subTipo} onChange={e => setSubTipo(e.target.value)} />
            </FG>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <FG label="Protocolo" optional>
              <input style={cx.input} value={protocolo} onChange={e => setProtocolo(e.target.value)} placeholder="Ej. Abdomen, Tórax..." />
            </FG>
            <FG label="Prioridad">
              <select style={cx.input} value={prioridad} onChange={e => setPrioridad(e.target.value)}>
                <option value="Rutina">Rutina</option>
                <option value="urgente">Urgente</option>
              </select>
            </FG>
          </div>
          <FG label="Creatinina" optional>
            <input style={cx.input} value={creatinina} onChange={e => setCreatinina(e.target.value)} placeholder="Ej. 0.9" />
          </FG>
        </div>

        {/* Médico solicitante — editable */}
        <div style={{ background: "#FFF7ED", borderRadius: 8, padding: "12px 14px", marginBottom: 16 }}>
          <p style={{ margin: "0 0 10px", fontSize: 11, fontWeight: 700, color: "#92400E", letterSpacing: "0.06em" }}>👨‍⚕️ MÉDICO SOLICITANTE</p>
          <FG label="Nombre del médico">
            <input style={cx.input} value={nombreMedico} onChange={e => setNombreMedico(e.target.value)} />
          </FG>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <FG label="MIP / Residente" optional>
              <input style={cx.input} value={mipSolicitante} onChange={e => setMipSolicitante(e.target.value)} />
            </FG>
            <FG label="Teléfono">
              <input style={cx.input} value={telefono} onChange={e => setTelefono(e.target.value)} />
            </FG>
          </div>
        </div>
...
function CensoView({ solicitudes, loading, token, onSelectSolicitud, adscritos, perfiles }) {
  ...
  // Agrupar por turno → adscrito
  const groupedByTurnoAdscrito = {};
  ...
  list.forEach(sol => {
    ...
    if (!groupedByTurnoAdscrito[turno]) groupedByTurnoAdscrito[turno] = {};
    if (!groupedByTurnoAdscrito[turno][doctor]) groupedByTurnoAdscrito[turno][doctor] = [];
    
    groupedByTurnoAdscrito[turno][doctor].push(sol);
  });
  ...
  const turnosPresentes = turnoOrder.filter(t => groupedByTurnoAdscrito[t]);

  // Renumerar en orden visual (turno → adscrito → solicitudes)
  let rowNum = 1;
  const allTurnos = [...turnosPresentes];
  if (groupedByTurnoAdscrito["Sin asignar"]) allTurnos.push("Sin asignar");
  allTurnos.forEach(turno => {
    Object.values(groupedByTurnoAdscrito[turno]).forEach(estudios => {
      estudios.forEach(sol => {
        sol.rowNum = rowNum++;
      });
    });
  });
  ...
  return (
    <div style={{ padding: "20px", maxWidth: 1600, margin: "0 auto", background: "#F8FAFC", minHeight: "100vh" }}>
      ...
      {/* Selector de Fecha */}
      <div style={{ marginBottom: "20px", display: "flex", gap: "12px", alignItems: "center", justifyContent: "center", flexWrap: "wrap" }}>
        ...
        <button
          onClick={() => {
            ...
            const allT = [...turnosPresentes];
            if (groupedByTurnoAdscrito["Sin asignar"]) allT.push("Sin asignar");

            let html = `<html xmlns:x="urn:schemas-microsoft-com:office:excel"><head><meta charset="UTF-8"><style>
              table { border-collapse: collapse; font-family: Arial, sans-serif; font-size: 12px; }
              td, th { border: 1px solid #D8E0EA; padding: 10px 12px; vertical-align: top; }
              .turno { background: #DBEAFE; color: #1E3A8A; font-weight: bold; font-size: 15px; }
              .adscrito { background: #F8FAFC; color: #334155; font-weight: bold; font-size: 13px; }
              .hdr { background: #EEF2F7; color: #475569; font-weight: bold; text-align: center; }
              .realizado { background: #ECFDF5; color: #059669; font-weight: bold; text-align: center; }
              .pendiente { background: #FFFBEB; color: #B45309; font-weight: bold; text-align: center; }
              .urgente { color: #E11D48; font-weight: bold; }
            </style></head><body><table>`;

            html += `<tr><td colspan="10" style="background:#DBEAFE;color:#1E3A8A;font-size:16px;font-weight:bold;text-align:center;padding:12px;">IMAGENOLOG\u00cdA - CENSO ${esc(fecha)}</td></tr>`;
            html += `<tr><td colspan="10" style="text-align:center;color:#64748B;">Hospital General de Tampico "Dr. Carlos Canseco" — Jornada 7:00 AM \u2192 7:00 AM</td></tr>`;

            if (guardias.length > 0) {
              html += `<tr><td colspan="10" style="background:#FEF3C7;color:#92400E;font-weight:bold;">\ud83d\udee1\ufe0f GUARDIA: ${esc(guardias.map(g => g.nombre).join(", "))}</td></tr>`;
            }

            allT.forEach(turno => {
              html += `<tr><td colspan="10" class="turno">TURNO: ${esc(turno.toUpperCase())}</td></tr>`;
              Object.entries(groupedByTurnoAdscrito[turno]).forEach(([doctor, estudios]) => {
                html += `<tr><td colspan="10" class="adscrito">${esc(doctor)} (${estudios.length})</td></tr>`;
                html += `<tr>
                  <th class="hdr">No</th><th class="hdr">Paciente</th><th class="hdr">Sexo</th><th class="hdr">Edad</th>
                  <th class="hdr">Servicio</th><th class="hdr">Diagn\u00f3stico</th><th class="hdr">Estudio</th>
                  <th class="hdr">Protocolo</th><th class="hdr">Estatus</th><th class="hdr">Hallazgos</th>
                </tr>`;
                estudios.forEach(sol => {
                  const estadoLabel = ESTADOS_CFG[sol.estado]?.label || sol.estado || "-";
                  const estadoClass = sol.estado === "realizado" ? "realizado" : "pendiente";
                  const urgente = sol.prioridad === "urgente";
                  html += `<tr>
                    <td style="text-align:center;font-weight:bold;">${sol.rowNum}</td>
                    <td class="${urgente ? "urgente" : ""}" style="font-weight:bold;">${esc(sol.nombre_paciente?.toUpperCase())}</td>
                    <td style="text-align:center;">${esc(sol.sexo?.charAt(0).toUpperCase() || "-")}</td>
                    <td style="text-align:center;">${esc(sol.edad || "-")}</td>
                    <td>${esc((sol.servicio_solicitante || sol.servicio_otro || "-").toUpperCase())}</td>
                    <td>${esc((sol.motivo_clinico || "-").toUpperCase())}</td>
                    <td>${esc(sol.tipo_estudio?.toUpperCase())}</td>
                    <td>${esc((sol.protocolo || "-").toUpperCase())}</td>
                    <td class="${estadoClass}">${esc(estadoLabel)}</td>
                    <td>${esc(sol.hallazgos || "-")}</td>
                  </tr>`;
                });
              });
            });
            ...
            a.href = url;
            a.download = `Censo_${fecha}.xls`;
            a.click();
            URL.revokeObjectURL(url);
          }}
          style={{ padding: "8px 16px", background: "#16A34A", color: "white", border: "none", borderRadius: "6px", fontSize: "13px", fontWeight: "600", cursor: "pointer" }}
        >
          📥 Exportar Excel
        </button>
      </div>
      ...
      {/* Turnos */}
      {[...turnosPresentes, ...(groupedByTurnoAdscrito["Sin asignar"] ? ["Sin asignar"] : [])].map((turno, turnoIdx) => (
        <div key={turno} style={{ marginBottom: "30px" }}>
          {/* Header Turno */}
          <div style={{ background: turno === "Sin asignar" ? "#FFE4E6" : "#DBEAFE", color: turno === "Sin asignar" ? "#9F1239" : "#1E3A8A", padding: "14px 20px", borderRadius: "8px 8px 0 0", fontWeight: "bold", fontSize: "16px", marginBottom: "0px", letterSpacing: "0.5px" }}>
            {turno === "Sin asignar" ? "⚠️ SIN ASIGNAR" : `🕐 TURNO: ${turno.toUpperCase()}`}
          </div>

          {/* Adscritos dentro del turno */}
          {Object.entries(groupedByTurnoAdscrito[turno]).map(([doctor, estudios], docIdx) => (
            <div key={doctor} style={{ border: "1px solid #CBD5E1", borderTop: docIdx === 0 ? "1px solid #CBD5E1" : "none", background: "#FFF" }}>
              {/* Header Adscrito */}
              <div style={{ background: "#F8FAFC", padding: "12px 20px", fontWeight: "600", fontSize: "14px", color: "#334155", borderBottom: "1px solid #E2E8F0" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", tableLayout: "fixed" }}>
                ...
                <thead style={{ display: docIdx === 0 ? "table-header-group" : "none" }}>
                  <tr style={{ background: "#EEF2F7", color: "#475569" }}>
                    <th style={{ padding: "12px 10px", textAlign: "center", fontWeight: "bold", borderRight: "1px solid #E2E8F0" }}>No</th>
                    <th style={{ padding: "12px 10px", textAlign: "left", fontWeight: "bold", borderRight: "1px solid #E2E8F0" }}>Paciente</th>
                    <th style={{ padding: "12px 10px", textAlign: "center", fontWeight: "bold", borderRight: "1px solid #E2E8F0" }}>Sexo</th>
                    <th style={{ padding: "12px 10px", textAlign: "center", fontWeight: "bold", borderRight: "1px solid #E2E8F0" }}>Edad</th>
                    <th style={{ padding: "12px 10px", textAlign: "left", fontWeight: "bold", borderRight: "1px solid #E2E8F0" }}>Servicio</th>
                    <th style={{ padding: "12px 10px", textAlign: "left", fontWeight: "bold", borderRight: "1px solid #E2E8F0" }}>Diagnóstico</th>
                    <th style={{ padding: "12px 10px", textAlign: "left", fontWeight: "bold", borderRight: "1px solid #E2E8F0" }}>Estudio</th>
                    <th style={{ padding: "12px 10px", textAlign: "left", fontWeight: "bold", borderRight: "1px solid #E2E8F0" }}>Protocolo</th>
                    <th style={{ padding: "12px 10px", textAlign: "center", fontWeight: "bold", borderRight: "1px solid #E2E8F0" }}>Estatus</th>
                    <th style={{ padding: "12px 10px", textAlign: "left", fontWeight: "bold" }}>Hallazgos</th>
                  </tr>
                </thead>
                <tbody>
                  {estudios.map((sol, idx) => (
                    ...
                    >
                      <td style={{ padding: "12px 10px", textAlign: "center", fontWeight: "600", color: "#475569", borderRight: "1px solid #E2E8F0" }}>{sol.rowNum}</td>
                      <td style={{ padding: "12px 10px", textAlign: "left", fontWeight: "600", color: "#1E40AF", borderRight: "1px solid #E2E8F0", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{sol.nombre_paciente?.toUpperCase()}</td>
                      <td style={{ padding: "12px 10px", textAlign: "center", color: "#64748B", borderRight: "1px solid #EDF2F7" }}>{sol.sexo?.charAt(0).toUpperCase() || "-"}</td>
                      <td style={{ padding: "12px 10px", textAlign: "center", color: "#64748B", borderRight: "1px solid #EDF2F7" }}>{sol.edad || "-"}</td>
                      <td style={{ padding: "12px 10px", textAlign: "left", color: "#64748B", borderRight: "1px solid #EDF2F7", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "12px" }}>{(sol.servicio_solicitante || sol.servicio_otro || "-").toUpperCase()}</td>
                      <td style={{ padding: "12px 10px", textAlign: "left", color: "#64748B", borderRight: "1px solid #EDF2F7", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "12px" }}>{(sol.motivo_clinico || "-").toUpperCase()}</td>
                      <td style={{ padding: "12px 10px", textAlign: "left", color: "#64748B", borderRight: "1px solid #EDF2F7", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "12px" }}>{sol.tipo_estudio?.toUpperCase()}</td>
                      <td style={{ padding: "12px 10px", textAlign: "left", color: "#64748B", borderRight: "1px solid #EDF2F7", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "12px" }}>{(sol.protocolo || "-").toUpperCase()}</td>
                      <td style={{ padding: "12px 10px", textAlign: "center", color: "#64748B", borderRight: "1px solid #EDF2F7" }}>
                        <span style={{ background: ESTADOS_CFG[sol.estado]?.bg || "#E5E7EB", color: ESTADOS_CFG[sol.estado]?.color || "#374151", padding: "2px 6px", borderRadius: 4, fontSize: "11px", fontWeight: 600 }}>
                          {ESTADOS_CFG[sol.estado]?.label || sol.estado}
                        </span>
                      </td>
                      <td style={{ padding: "12px 10px", textAlign: "left", color: "#4B5563", fontSize: "12px", overflow: "hidden", textOverflow: "ellipsis" }}>
                        {sol.hallazgos ? sol.hallazgos.substring(0, 45) + (sol.hallazgos.length > 45 ? "..." : "") : "-"}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
function SolicitudEmergenciaForm({ token, perfil, adscritos, onSuccess }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const newSolicitud = {
        ...
        estado: "realizado",
        es_emergencia: false,
        adscrito_id: adscritoSeleccionado || (Array.isArray(adscritos) && adscritos[0]?.id) || perfil.id,
        nombre_medico: adscritoSeleccionado ? (adscritoObj?.nombre || "—") : ((Array.isArray(adscritos) && adscritos[0]?.nombre) || perfil.nombre || "—"),
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <FG label="Teléfono" required>
        <input type="tel" value={telefono} onChange={e => setTelefono(e.target.value)} placeholder="Teléfono de contacto" style={{ padding: "8px 12px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }} />
      </FG>
...
// ─── Panel Subdirector (Solicitudes con calendario, read-only) ─────────────────
function PanelSubdirectorSolicitudes({ session, onLogout }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setAds(Array.isArray(a) ? a.filter(ad => ad.activo) : []);
      setLoading(false);
    })();
  }, [token]);
  ...
  const todaySols = sols.filter(s => s.created_at && s.created_at >= todayStart && s.created_at <= todayEnd);

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20, padding: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700 }}>📋 Subdirector — Solicitudes</h1>
        <button onClick={onLogout} style={{ padding: "8px 16px", background: "#EF4444", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: 600 }}>
          Salir
        </button>
      </div>

      {/* Banner informativo */}
      <div style={{ background: "#FEF3C7", border: "1px solid #FCD34D", borderRadius: 8, padding: "12px 16px", color: "#92400E" }}>
        <p style={{ margin: 0, fontSize: 13, fontWeight: 600 }}>
          ℹ️ Tomar en cuenta que los 10-15 pacientes vistos diario en la consulta no son parte de este censo
        </p>
      </div>
      ...
      <div style={{ display: "grid", gap: 12, gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill, minmax(300px, 1fr))" }}>
        {todaySols.map(sol => (
          ...
          >
            <p style={{ margin: "0 0 8px", fontWeight: 700, fontSize: 15, color: "#1E40AF" }}>
              {sol.nombre_paciente}
            </p>
            ...
            <p style={{ margin: "0 0 4px", fontSize: 12, color: "#9CA3AF" }}>
              🏥 {sol.servicio_solicitante || sol.servicio_otro || "Otro"}
            </p>
            <span style={{ background: ESTADOS_CFG[sol.estado]?.bg || "#E5E7EB", color: ESTADOS_CFG[sol.estado]?.color || "#374151", padding: "2px 8px", borderRadius: 3, fontSize: 11, fontWeight: 600 }}>
              {ESTADOS_CFG[sol.estado]?.label || sol.estado}
            </span>
          </div>
        ))}
      ...
      {selected && <SolicitudModalReadOnly sol={selected} onClose={() => setSelected(null)} />}
    </div>
  );
}

// ─── Panel Jefe (read-only, mismo layout que PanelResidente) ───────────────────
function PanelJefeTodo({ session, onLogout }) {
  const { uid, token, perfil } = session;
  const isMobile = useIsMobile();
  ...
  const groupedByTurnoAdscrito = {};
  todaySols.forEach(sol => {
    const adscritoObj = Array.isArray(ads) ? ads.find(a => a.id === sol.adscrito_id) : null;
    const turno = adscritoObj?.turno_actual || "Sin asignar";
    const doctor = adscritoObj?.nombre || "Sin adscrito";
    if (!groupedByTurnoAdscrito[turno]) groupedByTurnoAdscrito[turno] = {};
    if (!groupedByTurnoAdscrito[turno][doctor]) groupedByTurnoAdscrito[turno][doctor] = [];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20, padding: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700 }}>📊 Jefe — Vista Completa</h1>
        <button onClick={onLogout} style={{ padding: "8px 16px", background: "#EF4444", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: 600 }}>
          Salir
        </button>
      </div>
      ...
      {/* Tabs */}
      <div style={{ display: "flex", gap: 12, borderBottom: "2px solid #E5E7EB", overflowX: "auto" }}>
        {[
        ...
        <div style={{ display: "grid", gap: 12, gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill, minmax(300px, 1fr))" }}>
          {todaySols.map(sol => (
            ...
            >
              ...
              <p style={{ margin: "0 0 4px", fontSize: 12, color: "#9CA3AF" }}>
                🏥 {sol.servicio_solicitante || sol.servicio_otro}
              </p>
              <span style={{ background: ESTADOS_CFG[sol.estado]?.bg, color: ESTADOS_CFG[sol.estado]?.color, padding: "2px 8px", borderRadius: 3, fontSize: 11, fontWeight: 600 }}>
                {ESTADOS_CFG[sol.estado]?.label}
              </span>
            </div>
          ))}
        </div>
      ...
      {view === "censo" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {[...turnosPresentes, ...(groupedByTurnoAdscrito["Sin asignar"] ? ["Sin asignar"] : [])].map(turno => (
            <div key={turno} style={{ marginBottom: "30px" }}>
              <div style={{ background: turno === "Sin asignar" ? "#FFE4E6" : "#DBEAFE", color: turno === "Sin asignar" ? "#9F1239" : "#1E3A8A", padding: "14px 20px", borderRadius: "8px 8px 0 0", fontWeight: "bold", fontSize: "16px" }}>
                {turno === "Sin asignar" ? "⚠️ SIN ASIGNAR" : `🕐 TURNO: ${turno.toUpperCase()}`}
              </div>
              ...
              {Object.entries(groupedByTurnoAdscrito[turno]).map(([doctor, estudios], docIdx) => (
                <div key={doctor} style={{ border: "1px solid #CBD5E1", borderTop: docIdx === 0 ? "1px solid #CBD5E1" : "none", background: "#FFF" }}>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px", tableLayout: "fixed" }}>
                    ...
                    <thead style={{ display: docIdx === 0 ? "table-header-group" : "none" }}>
                      <tr style={{ background: "#EEF2F7", color: "#475569" }}>
                        <th style={{ padding: "12px 10px", textAlign: "center", fontWeight: "bold", borderRight: "1px solid #EDF2F7" }}>No</th>
                        <th style={{ padding: "12px 10px", textAlign: "left", fontWeight: "bold", borderRight: "1px solid #EDF2F7" }}>Paciente</th>
                        <th style={{ padding: "12px 10px", textAlign: "center", fontWeight: "bold", borderRight: "1px solid #EDF2F7" }}>Sexo</th>
                        <th style={{ padding: "12px 10px", textAlign: "center", fontWeight: "bold", borderRight: "1px solid #EDF2F7" }}>Edad</th>
                        <th style={{ padding: "12px 10px", textAlign: "left", fontWeight: "bold", borderRight: "1px solid #EDF2F7" }}>Servicio</th>
                        <th style={{ padding: "12px 10px", textAlign: "left", fontWeight: "bold", borderRight: "1px solid #EDF2F7" }}>Diagnóstico</th>
                        <th style={{ padding: "12px 10px", textAlign: "left", fontWeight: "bold", borderRight: "1px solid #EDF2F7" }}>Estudio</th>
                        <th style={{ padding: "12px 10px", textAlign: "left", fontWeight: "bold", borderRight: "1px solid #EDF2F7" }}>Protocolo</th>
                        <th style={{ padding: "12px 10px", textAlign: "center", fontWeight: "bold", borderRight: "1px solid #EDF2F7" }}>Estatus</th>
                        <th style={{ padding: "12px 10px", textAlign: "left", fontWeight: "bold" }}>Hallazgos</th>
                      </tr>
                    </thead>
                    <tbody>
                      {estudios.map((sol, idx) => (
                        <tr
                          ...
                          onMouseLeave={e => e.currentTarget.style.background = idx % 2 === 0 ? "#FAFBFC" : "#FFF"}
                        >
                          <td style={{ padding: "12px 10px", textAlign: "center", fontWeight: "600", color: "#475569", borderRight: "1px solid #EDF2F7" }}>{idx + 1}</td>
                          <td style={{ padding: "12px 10px", textAlign: "left", fontWeight: "600", color: "#1E40AF", borderRight: "1px solid #EDF2F7", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{sol.nombre_paciente?.toUpperCase()}</td>
                          <td style={{ padding: "12px 10px", textAlign: "center", color: "#64748B", borderRight: "1px solid #EDF2F7" }}>{sol.sexo?.charAt(0).toUpperCase() || "-"}</td>
                          <td style={{ padding: "12px 10px", textAlign: "center", color: "#64748B", borderRight: "1px solid #EDF2F7" }}>{sol.edad || "-"}</td>
                          <td style={{ padding: "12px 10px", textAlign: "left", color: "#64748B", borderRight: "1px solid #EDF2F7", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "12px" }}>{(sol.servicio_solicitante || sol.servicio_otro || "-").toUpperCase()}</td>
                          <td style={{ padding: "12px 10px", textAlign: "left", color: "#64748B", borderRight: "1px solid #EDF2F7", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "12px" }}>{(sol.motivo_clinico || "-").toUpperCase()}</td>
                          ...
                          <td style={{ padding: "12px 10px", textAlign: "left", color: "#64748B", borderRight: "1px solid #EDF2F7", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontSize: "12px" }}>{(sol.protocolo || "-").toUpperCase()}</td>
                          <td style={{ padding: "12px 10px", textAlign: "center", color: "#64748B", borderRight: "1px solid #EDF2F7" }}>
                            <span style={{ background: ESTADOS_CFG[sol.estado]?.bg, color: ESTADOS_CFG[sol.estado]?.color, padding: "2px 6px", borderRadius: 4, fontSize: "11px", fontWeight: 600 }}>
                              {ESTADOS_CFG[sol.estado]?.label}
                            </span>
                          </td>
                          <td style={{ padding: "12px 10px", textAlign: "left", color: "#64748B", fontSize: "10px", overflow: "hidden", textOverflow: "ellipsis" }}>
                            {sol.hallazgos ? sol.hallazgos.substring(0, 45) + (sol.hallazgos.length > 45 ? "..." : "") : "-"}
                          </td>
                        </tr>
                      ))}
...
// ─── Modal Read-Only para Subdirector y Jefe ──────────────────────────────
function SolicitudModalReadOnly({ sol, onClose }) {
      ...
      <div style={{ ...cx.card, width: "100%", maxWidth: 560, maxHeight: "92vh", overflowY: "auto" }}>
        ...
        {sol.hallazgos && (
          <div style={{ background: "#F0FDF4", borderRadius: 8, padding: "12px 14px", marginBottom: 16 }}>
            <p style={{ margin: "0 0 6px", fontSize: 11, fontWeight: 700, color: "#15803D" }}>HALLAZGOS</p>
            <p style={{ margin: 0, fontSize: 13, color: "#374151", whiteSpace: "pre-wrap" }}>{sol.hallazgos}</p>
          </div>
        )}

        {sol.notas && (
          <div style={{ background: "#FEE2E2", borderRadius: 8, padding: "12px 14px", marginBottom: 16 }}>
            <p style={{ margin: "0 0 6px", fontSize: 11, fontWeight: 700, color: "#DC2626" }}>📝 NOTAS / MOTIVO</p>
            <p style={{ margin: 0, fontSize: 13, color: "#374151", whiteSpace: "pre-wrap" }}>{sol.notas}</p>
          </div>
        )}

        {sol.horario_programado && (
          <div style={{ background: "#EFF6FF", borderRadius: 8, padding: "12px 14px", marginBottom: 16 }}>
            <p style={{ margin: "0 0 6px", fontSize: 11, fontWeight: 700, color: "#1E40AF" }}>🕐 HORARIO PROGRAMADO</p>
            <p style={{ margin: 0, fontSize: 13, color: "#374151" }}>{sol.horario_programado}</p>
          </div>
        )}

        <button onClick={onClose} style={{ width: "100%", padding: "10px", background: "#3B82F6", color: "white", border: "none", borderRadius: 6, fontWeight: 600, cursor: "pointer" }}>
          Cerrar
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  if (session.perfil.rol === "subdirector") {
    console.log("Rendering PanelSubdirectorSolicitudes");
    return <PanelSubdirectorSolicitudes session={session} onLogout={handleLogout} />;
  }

  if (session.perfil.rol === "jefe") {
    console.log("Rendering PanelJefeTodo");
    return <PanelJefeTodo session={session} onLogout={handleLogout} />;
  }
