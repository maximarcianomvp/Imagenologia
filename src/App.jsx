...
function PanelResidente({ session, onLogout }) {
  ...
  const load = useCallback(async () => {
    ...
    if (prevCount.current > 0 && newSols.length > prevCount.current) playNotif();
    ...
    <div style={{ minHeight: "100vh", background: "#F8FAFC" }}>
      ...
      <div style={{ background: "#1E3A8A", padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {TABS.map(tab => (
            <button key={tab.id} onClick={() => setView(tab.id)} style={{ padding: "6px 12px", borderRadius: 7, border: "1px solid", cursor: "pointer", fontSize: 12, fontFamily: "inherit", background: view === tab.id ? "#fff" : "transparent", color: view === tab.id ? "#1E3A8A" : "#93C5FD", borderColor: view === tab.id ? "#fff" : "#3B82F6", fontWeight: view === tab.id ? 700 : 400 }}>
