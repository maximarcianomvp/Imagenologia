...
const cx = {
  input: {
    width: "100%", padding: "8px 8px", border: "1px solid #D1D5DB",
...
function Badge({ label, bg, color }) {
  return <span style={{ background: bg, color, padding: "2px 9px", borderRadius: 20, fontSize: 11, fontWeight: 600, whiteSpace: "normal" }}>{label}</span>;
}
...
function SolicitudModal({ sol, adscritos, token, onUpdate, onClose }) {
  ...
  async function save() {
    setBusy(true);
    await db.updateSolicitud(sol.id, { nss_expediente: exp, adscrito_id: adscrito || null, estado, horario_programado: horario, notas, hallazgos, protocolo, fecha_recordatorio: fechaRecordatorio || null }, token);
    await onUpdate();
    setBusy(false);
    onClose();
  }
      ...
      <div style={{ ...cx.card, width: "100%", maxWidth: 560, maxHeight: "92vh", overflowY: "auto" }}>
        ...
        <div style={{ background: "#F8FAFC", borderRadius: 8, padding: "12px 14px", marginBottom: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 4, fontSize: 13, color: "#374151" }}>
            ...
            <span>Sexo: {sol.sexo?.charAt(0).toUpperCase() || "—"}</span>
        ...
        <div style={{ borderTop: "1px solid #E5E7EB", paddingTop: 16 }}>
          ...
          <FG label="Protocolo" optional>
            <input style={cx.input} value={protocolo} onChange={e => setProtocolo(e.target.value)} placeholder="Ej. abdomen superior, columna lumbar..." />
          </FG>
    ...
    <div style={{ padding: "20px", maxWidth: 1600, margin: "0 auto", background: "#F8FAFC", minHeight: "100vh" }}>
      ...
      <div style={{ marginBottom: "20px", display: "flex", gap: "12px", alignItems: "center", justifyContent: "center" }}>
        ...
        <input
          ...
          style={{ padding: "8px 8px", borderRadius: "6px", border: "1px solid #CBD5E1", fontSize: "13px", fontFamily: "inherit" }}
        />
      </div>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "11px", tableLayout: "fixed" }}>
                    ...
                    <thead style={{ display: docIdx === 0 && servIdx === 0 ? "table-header-group" : "none" }}>
                      <tr style={{ background: "#374151", color: "white" }}>
                        <th style={{ padding: "10px 8px", textAlign: "center", fontWeight: "bold", minWidth: "35px", borderRight: "1px solid #1F2937", minWidth: "35px" }}>No</th>
                        <th style={{ padding: "10px 8px", textAlign: "left", fontWeight: "bold", borderRight: "1px solid #1F2937" }}>Paciente</th>
                        <th style={{ padding: "10px 8px", textAlign: "center", fontWeight: "bold", minWidth: "45px", borderRight: "1px solid #1F2937" }}>Sexo</th>
                        <th style={{ padding: "10px 8px", textAlign: "center", fontWeight: "bold", minWidth: "45px", borderRight: "1px solid #1F2937" }}>Edad</th>
                        ...
                        <th style={{ padding: "10px 8px", textAlign: "center", fontWeight: "bold", borderRight: "1px solid #1F2937" }}>Estatus</th>
                        <th style={{ padding: "10px 8px", textAlign: "left", fontWeight: "bold" }}>Hallazgos</th>
                      </tr>
                    </thead>
                    <tbody>
                      {estudios.map((sol, idx) => (
                        ...
                        >
                          ...
                          <td style={{ padding: "8px 8px", textAlign: "left", fontWeight: "600", color: "#1E40AF", borderRight: "1px solid #E2E8F0" }}>{sol.nombre_paciente?.toUpperCase()}</td>
                          <td style={{ padding: "8px 8px", textAlign: "center", color: "#4B5563", borderRight: "1px solid #E2E8F0" }}>{sol.sexo?.charAt(0).toUpperCase() || "-"}</td>
                          <td style={{ padding: "8px 8px", textAlign: "center", color: "#4B5563", borderRight: "1px solid #E2E8F0" }}>{sol.edad || "-"}</td>
                          <td style={{ padding: "8px 8px", textAlign: "left", color: "#4B5563", borderRight: "1px solid #E2E8F0", fontSize: "10px" }}>{sol.servicio_solicitante?.toUpperCase() || sol.servicio_otro || "-"}</td>
                          <td style={{ padding: "8px 8px", textAlign: "left", color: "#4B5563", borderRight: "1px solid #E2E8F0", fontSize: "10px" }}>{sol.motivo_clinico?.toUpperCase() || "-"}</td>
                          <td style={{ padding: "8px 8px", textAlign: "left", color: "#4B5563", borderRight: "1px solid #E2E8F0", fontSize: "10px" }}>{sol.tipo_estudio?.toUpperCase()}</td>
                          <td style={{ padding: "8px 8px", textAlign: "left", color: "#4B5563", borderRight: "1px solid #E2E8F0", fontSize: "10px" }}>{sol.tipo_estudio?.toUpperCase()}</td>
                          <td style={{ padding: "8px 8px", textAlign: "left", color: "#4B5563", borderRight: "1px solid #E2E8F0", overflow: "hidden", textOverflow: "ellipsis" }}>{sol.protocolo?.toUpperCase() || "-"}</td>
                          <td style={{ padding: "8px 8px", textAlign: "center", color: "#4B5563", borderRight: "1px solid #E2E8F0", fontSize: "10px" }}>
                            <span style={{ background: ESTADOS_CFG[sol.estado]?.bg || "#E5E7EB", color: ESTADOS_CFG[sol.estado]?.color || "#374151", padding: "2px 6px", borderRadius: 3, fontSize: "9px", fontWeight: 600 }}>
                              {ESTADOS_CFG[sol.estado]?.label || sol.estado}
                            </span>
                          </td>
                          <td style={{ padding: "8px 8px", textAlign: "left", color: "#4B5563", fontSize: "10px" }}>
      ...
      {groupedByTurnoServicio["Sin asignar"] && Object.keys(groupedByTurnoServicio["Sin asignar"]).length > 0 && (
        <div style={{ marginBottom: "30px" }}>
          ...
          {Object.entries(groupedByTurnoServicio["Sin asignar"]).map(([servicio, doctoresObj], servIdx) => (
            <div key={servicio} style={{ border: "1px solid #CBD5E1", borderTop: servIdx === 0 ? "1px solid #CBD5E1" : "none", background: "#FFF" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {data.map(([label, val]) => (
          <div key={label}>
            ...
            <div style={{ background: "#F3F4F6", borderRadius: 6, height: 10, overflow: "hidden", textOverflow: "ellipsis" }}>
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <FG label="Título" required>
        <input type="text" value={titulo} onChange={e => setTitulo(e.target.value)} placeholder="Ej: Mantenimiento del equipo" style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }} />
      </FG>

      <FG label="Descripción" optional>
        <textarea value={descripcion} onChange={e => setDescripcion(e.target.value)} placeholder="Detalles adicionales..." rows={3} style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%", resize: "none" }} />
      </FG>

      <FG label="Imagen" required>
        <input type="file" accept="image/*" onChange={e => setFile(e.target.files?.[0] || null)} style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }} />
        {file && <p style={{ margin: "8px 0 0", fontSize: 12, color: "#10B981" }}>✓ {file.name}</p>}
      </FG>

      {msg && (
        <div style={{ padding: "10px 8px", borderRadius: 6, background: msg.type === "error" ? "#FEE2E2" : "#DCFCE7", color: msg.type === "error" ? "#DC2626" : "#16A34A", fontSize: 13, fontWeight: 500 }}>
          {msg.text}
        </div>
      )}
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <FG label="Adscrito responsable" required>
        <select value={adscritoSeleccionado} onChange={e => setAdscritoSeleccionado(e.target.value)} style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <FG label="Fecha de registro" required>
          <input type="date" value={fechaRegistro} onChange={e => setFechaRegistro(e.target.value)} style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }} />
        </FG>
        <FG label="Hora de registro" required>
          <input type="time" value={horaRegistro} onChange={e => setHoraRegistro(e.target.value)} style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }} />
        </FG>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 12 }}>
        <FG label="Paciente" required>
          <input type="text" value={nombrePaciente} onChange={e => setNombrePaciente(e.target.value)} placeholder="Nombre completo" style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }} />
        </FG>
        <FG label="Edad" required>
          <input type="number" value={edad} onChange={e => setEdad(e.target.value)} placeholder="Años" min="0" max="120" style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }} />
        </FG>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <FG label="Tipo de estudio" required>
          <select value={tipoEstudio} onChange={e => setTipoEstudio(e.target.value)} style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }}>
            <option>Ultrasonido</option>
        ...
        <FG label="Sub-tipo" optional>
          <input type="text" value={subTipo} onChange={e => setSubTipo(e.target.value)} placeholder="Especificación" style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }} />
        </FG>
      </div>

      <FG label="Servicio solicitante" required>
        <select value={servicio} onChange={e => setServicio(e.target.value)} style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }}>
          {SERVICIOS.map(s => (
      ...
      {servicio === "Otro" && (
        <FG label="Especificar servicio" required>
          <input type="text" value={servicioOtro} onChange={e => setServicioOtro(e.target.value)} placeholder="Nombre del servicio" style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }} />
        </FG>
      )}

      <FG label="Hallazgos" optional>
        <textarea value={hallazgos} onChange={e => setHallazgos(e.target.value)} placeholder="Descripción de hallazgos encontrados" rows={3} style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%", resize: "none" }} />
      </FG>

      <FG label="MIP/Residente solicitante" optional>
        <input type="text" value={mipSolicitante} onChange={e => setMipSolicitante(e.target.value)} placeholder="Nombre del MIP o residente que solicita" style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }} />
      </FG>
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      <FG label="Adscrito a cargo" optional>
        <select value={adscritoId} onChange={e => setAdscritoId(e.target.value)} style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%" }}>
      ...
      <FG label="Hallazgos" optional>
        <textarea value={hallazgos} onChange={e => setHallazgos(e.target.value)} placeholder="Descripción de hallazgos" rows={3} style={{ padding: "8px 8px", borderRadius: 6, border: "1px solid #D1D5DB", fontSize: 14, fontFamily: "inherit", width: "100%", resize: "none" }} />
      </FG>
        ...
        {isMobile ? (
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 6, overflowX: "auto", paddingBottom: 2 }}>
              {TABS.map(tab => (
                <button key={tab.id} onClick={() => setView(tab.id)} style={{ padding: "6px 12px", borderRadius: 7, border: "1px solid", cursor: "pointer", fontSize: 12, fontFamily: "inherit", whiteSpace: "normal", flexShrink: 0, background: view === tab.id ? "#fff" : "transparent", color: view === tab.id ? "#1E3A8A" : "#93C5FD", borderColor: view === tab.id ? "#fff" : "#3B82F6", fontWeight: view === tab.id ? 700 : 400 }}>
          ...
          <>
            ...
            <div style={{ ...cx.card, marginBottom: 14 }}>
              ...
              <input
                ...
                style={{ padding: "8px 8px", borderRadius: 8, border: "1px solid #E5E7EB", fontSize: 14, fontFamily: "inherit", width: "100%", maxWidth: 200 }}
              />
            </div>
                ...
                {filtered.map(sol => {
                  ...
                  return (
                    <div key={sol.id} onClick={() => setSelected(sol)} style={{ ...cx.card, cursor: "pointer" }}>
                      ...
                      <p style={{ margin: "0 0 6px", fontSize: 14, fontWeight: 600 }}>{sol.nombre_paciente?.toUpperCase()} · {sol.edad ? `${sol.edad} años` : "—"} · {sol.sexo?.charAt(0).toUpperCase() || "—"}</p>
