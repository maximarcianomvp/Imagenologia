...
function CensoView({ solicitudes, loading, token, onSelectSolicitud, adscritos, perfiles }) {
  ...
  function exportToExcel() {
    try {
      ...
      // Título
      csvLines.push("CENSO DE IMAGENOLOGÍA");
      csvLines.push(`Jornada: ${fecha} 7:00 AM → ${new Date(new Date(fecha).getTime() + 86400000).toISOString().split('T')[0]} 7:00 AM`);
      csvLines.push("");

      // Guardias
      if (guardias.length > 0) {
        csvLines.push("RESIDENTES EN GUARDIA");
        guardias.forEach(g => csvLines.push(g.nombre));
        csvLines.push("");
      }
      ...
      turnosPresentes.forEach(turno => {
        csvLines.push(`TURNO: ${turno}`);
        csvLines.push(["Adscrito", "Paciente", "Edad", "Estudio", "Servicio", "Prioridad", "Estado", "Hallazgos"].join(","));
      ...
      // Crear y descargar CSV (compatible con Excel)
      ...
      link.setAttribute("download", `Censo_${fecha}.csv`);
