...
// ─── Panel Subdirector (solo lectura Solicitudes) ──────────────────────────────────────────
function PanelSubdirector({ session, onLogout }) {
  ...
  useEffect(() => {
    (async () => {
      const data = await db.getSolicitudes(token);
      setSols(data);
    })();
  }, [token]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, padding: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700 }}>📋 Solicitudes</h1>
        <button onClick={onLogout} style={{ padding: "8px 16px", background: "#EF4444", color: "white", border: "none", borderRadius: 6, cursor: "pointer" }}>
          Salir
        </button>
      </div>
      ...
      <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))" }}>
        {sols.map(sol => (
          ...
          >
            ...
            <span style={{ background: "#ECFDF5", color: "#059669", padding: "2px 8px", borderRadius: 3, fontSize: 11, fontWeight: 600 }}>
              {ESTADOS_CFG[sol.estado]?.label || sol.estado}
            </span>
          </div>
        ))}
...
function PanelJefeTodo({ session, onLogout }) {
  ...
  useEffect(() => {
    (async () => {
      const data = await db.getSolicitudes(token);
      setSols(data);
      const anuncios = await db.getAnuncios(token);
      setAds(anuncios || []);
    })();
  }, [token]);

  // Agrupar por turno → adscrito (simple)
  const groupedByTurnoAdscrito = {};
  const list = sols; // Sin filtros complejos - mostrar todas
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, padding: 16 }}>
      ...
      {/* Tabs */}
      <div style={{ display: "flex", gap: 8, borderBottom: "2px solid #E5E7EB", overflowX: "auto" }}>
        {[
      ...
      {/* Estadísticas Tab */}
      {view === "estadisticas" && (
        <div style={{ display: "grid", gap: 16, gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fit, minmax(250px, 1fr))" }}>
          <div style={{ background: "#EFF6FF", borderRadius: 8, padding: 16 }}>
            <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: "#1E40AF" }}>TOTAL SOLICITUDES</p>
            <p style={{ margin: "8px 0 0", fontSize: 28, fontWeight: 700, color: "#1E40AF" }}>{list.length}</p>
          </div>
          <div style={{ background: "#F0FDF4", borderRadius: 8, padding: 16 }}>
            <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: "#15803D" }}>REALIZADAS</p>
            <p style={{ margin: "8px 0 0", fontSize: 28, fontWeight: 700, color: "#15803D" }}>{list.filter(s => s.estado === "realizado").length}</p>
          </div>
          <div style={{ background: "#FFFBEB", borderRadius: 8, padding: 16 }}>
            <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: "#B45309" }}>PENDIENTES</p>
            <p style={{ margin: "8px 0 0", fontSize: 28, fontWeight: 700, color: "#B45309" }}>{list.filter(s => s.estado === "pendiente").length}</p>
          </div>
          <div style={{ background: "#FEE2E2", borderRadius: 8, padding: 16 }}>
            <p style={{ margin: 0, fontSize: 12, fontWeight: 700, color: "#DC2626" }}>URGENTES</p>
            <p style={{ margin: "8px 0 0", fontSize: 28, fontWeight: 700, color: "#DC2626" }}>{list.filter(s => s.prioridad === "urgente").length}</p>
          </div>
        </div>
      )}
      ...
      {view === "anuncios" && (
        <div style={{ display: "grid", gap: 12 }}>
          {ads.length === 0 ? (
            <p style={{ color: "#9CA3AF", textAlign: "center", padding: 20 }}>No hay anuncios</p>
          ) : (
            ads.map(anuncio => (
              <div key={anuncio.id} style={{ background: "#FFF", border: "1px solid #E5E7EB", borderRadius: 8, padding: 12 }}>
                <p style={{ margin: "0 0 4px", fontWeight: 700, fontSize: 14 }}>{anuncio.titulo}</p>
                <p style={{ margin: "0 0 8px", fontSize: 13, color: "#6B7280" }}>{anuncio.contenido}</p>
                <p style={{ margin: 0, fontSize: 11, color: "#9CA3AF" }}>
                  {new Date(anuncio.created_at).toLocaleString("es-MX")}
                </p>
              </div>
            ))
          )}
