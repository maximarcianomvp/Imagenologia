...
function CensoView({ solicitudes, loading, token, onSelectSolicitud, adscritos, perfiles }) {
  ...
  return (
    <div style={{ padding: 20 }}>
      <h2>📊 Censo de Imagenología</h2>
      <div style={{ marginBottom: 20 }}>
        <label>Fecha: </label>
        <input type="date" value={fecha} onChange={e => setFecha(e.target.value)} />
      </div>
      <div style={{ background: "#F3F4F6", padding: 15, borderRadius: 8 }}>
        <p>Total solicitudes: {Array.isArray(solicitudes) ? solicitudes.length : 0}</p>
        <p>Adscritos activos: {Array.isArray(adscritos) ? adscritos.length : 0}</p>
      </div>
    </div>
  );
}
