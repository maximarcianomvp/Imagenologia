...
function CensoView({ solicitudes, loading }) {
  ...
  const turnoOrder = ["Matutino", "Vespertino", "Nocturno"];
      ...
      html += `
        <tr>
          <td colspan="8" style="background:#1E3A8A;color:white;font-weight:bold;font-size:13px;padding:11px 16px;">
            ${turno === "Matutino" ? "🌅" : turno === "Vespertino" ? "🌤️" : turno === "Nocturno" ? "🌙" : "❓"} ${turno}
          </td>
        </tr>
      `;
            ...
            <div key={turno}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, paddingBottom: 12, borderBottom: "2px solid #E5E7EB" }}>
                <span style={{ fontSize: 20 }}>
                  {turno === "Matutino" ? "🌅" : turno === "Vespertino" ? "🌤️" : turno === "Nocturno" ? "🌙" : "❓"}
                </span>
          ...
          {adscritos.map(ads => (
            <div key={ads.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 14px", borderRadius: 8, background: "#F9FAFB", border: "1px solid #E5E7EB" }}>
              ...
              <select
                ...
                onChange={async (e) => {
                  setToggling(ads.id);
                  await db.updateAdscrito(ads.id, { turno_actual: e.target.value }, token);
                  setAdscritos(prev => prev.map(a => a.id === ads.id ? { ...a, turno_actual: e.target.value } : a));
                  setToggling(null);
                }}
