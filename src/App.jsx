...
function CensoView({ solicitudes, loading, token, onSelectSolicitud }) {
  ...
  function exportExcel() {
    ...
    // Recalcular list filtrada por fecha
    const { startISO, endISO } = getJornadaRange(fecha);
    const listExcel = solicitudes.filter(s => {
      if (!s.created_at) return false;
      const sTime = new Date(s.created_at);
      return sTime >= new Date(startISO) && sTime < new Date(endISO);
    });
    
    // Recalcular groupedByTurno
    const grouped = {};
    [...listExcel]
      .sort((a, b) => new Date(a.created_at) - new Date(b.created_at))
      .forEach(sol => {
    ...
    const emergencias = listExcel.filter(s => s.es_emergencia);
    if (emergencias.length > 0) {
      html += `
        <tr>
          <td colspan="7" style="background:#DC2626;color:white;font-weight:bold;font-size:13px;padding:11px 16px;height:25px;">
            🚨 SOLICITUD DE EMERGENCIA
          </td>
        </tr>
        <tr style="background:#FEE2E2;height:30px;">
          <th style="padding:7px 12px;border:1px solid #FCA5A5;color:#DC2626;font-size:10px;text-transform:uppercase;letter-spacing:1px;vertical-align:middle;">Paciente</th>
          <th style="padding:7px 12px;border:1px solid #FCA5A5;color:#DC2626;font-size:10px;text-transform:uppercase;letter-spacing:1px;vertical-align:middle;">Edad</th>
          <th style="padding:7px 12px;border:1px solid #FCA5A5;color:#DC2626;font-size:10px;text-transform:uppercase;letter-spacing:1px;vertical-align:middle;">Estudio</th>
          <th style="padding:7px 12px;border:1px solid #FCA5A5;color:#DC2626;font-size:10px;text-transform:uppercase;letter-spacing:1px;vertical-align:middle;">Servicio</th>
          <th style="padding:7px 12px;border:1px solid #FCA5A5;color:#DC2626;font-size:10px;text-transform:uppercase;letter-spacing:1px;vertical-align:middle;">Prioridad</th>
          <th style="padding:7px 12px;border:1px solid #FCA5A5;color:#DC2626;font-size:10px;text-transform:uppercase;letter-spacing:1px;vertical-align:middle;">Médico</th>
          <th style="padding:7px 12px;border:1px solid #FCA5A5;color:#DC2626;font-size:10px;text-transform:uppercase;letter-spacing:1px;vertical-align:middle;">Hallazgos</th>
        </tr>
      `;
      emergencias.forEach((sol, idx) => {
        const svc = sol.servicio_solicitante === "Otro" ? sol.servicio_otro : sol.servicio_solicitante;
        const P = PRIORIDADES_CFG[sol.prioridad] || PRIORIDADES_CFG.Urgencia;
        const rowBg = idx % 2 === 0 ? "#ffffff" : "#FEE2E2";
        html += `
          <tr style="background:${rowBg};height:40px;">
            <td style="padding:8px 12px;border:1px solid #FCA5A5;font-weight:600;vertical-align:middle;">${sol.nombre_paciente}</td>
            <td style="padding:8px 12px;border:1px solid #FCA5A5;vertical-align:middle;">${sol.edad || "—"}</td>
            <td style="padding:8px 12px;border:1px solid #FCA5A5;vertical-align:middle;">${sol.tipo_estudio}</td>
            <td style="padding:8px 12px;border:1px solid #FCA5A5;vertical-align:middle;">${svc}</td>
            <td style="padding:8px 12px;border:1px solid #FCA5A5;background:${P.bg};color:${P.color};font-weight:bold;vertical-align:middle;">${sol.prioridad}</td>
            <td style="padding:8px 12px;border:1px solid #FCA5A5;vertical-align:middle;">${sol.nombre_medico || "—"}</td>
            <td style="padding:8px 12px;border:1px solid #FCA5A5;color:#374151;vertical-align:middle;">${sol.hallazgos || "—"}</td>
          </tr>
        `;
      });
      html += `<tr><td colspan="7" style="padding:6px;height:12px;"></td></tr>`;
    ...
    const consultasExternas = listExcel.filter(s => s.es_consulta_externa);
  ...
  return (
    <div>
      ...
      {/* Solicitud de Emergencia */}
      {loading ? null : (() => {
        const emergencias = list.filter(s => s.es_emergencia);
        return emergencias.length > 0 ? (
          <div style={{ background: "#FEE2E2", borderRadius: 12, border: "1.5px solid #FCA5A5", padding: "16px", marginBottom: 24 }}>
            <p style={{ margin: "0 0 16px", fontSize: 13, fontWeight: 700, color: "#DC2626", textTransform: "uppercase", letterSpacing: "0.05em" }}>🚨 Solicitud de Emergencia ({emergencias.length})</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {emergencias.map(sol => {
                const svc = sol.servicio_solicitante === "Otro" ? sol.servicio_otro : sol.servicio_solicitante;
                const P = PRIORIDADES_CFG[sol.prioridad] || PRIORIDADES_CFG.Urgencia;
                return (
                  <div key={sol.id} style={{ background: "#fff", borderRadius: 8, padding: "12px", border: "1px solid #FCA5A5", cursor: "pointer" }} onClick={() => onSelectSolicitud(sol)}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                      <div>
                        <p style={{ margin: "0 0 4px", fontSize: 14, fontWeight: 700, color: "#DC2626" }}>{sol.nombre_paciente}</p>
                    ...
                    <p style={{ margin: "0 0 4px", fontSize: 12, color: "#374151" }}>👨‍⚕️ {sol.nombre_medico} · {svc}</p>
                    {sol.mip_solicitante && <p style={{ margin: 0, fontSize: 11, color: "#6B7280" }}>MIP: {sol.mip_solicitante}</p>}
                  </div>
                );
              })}
            </div>
                        ...
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 700 }}>
                          ...
                          <tbody>
                            {group.estudios.map((sol, i) => {
                              ...
                              const svc = sol.servicio_solicitante === "Otro" ? sol.servicio_otro : sol.servicio_solicitante;
                              return (
                                <tr key={sol.id} style={{ background: i % 2 === 0 ? "#fff" : clr.light, borderBottom: `1px solid ${clr.border}66`, cursor: "pointer" }} onClick={() => onSelectSolicitud(sol)}>
                                  <td style={{ padding: "8px 12px", fontWeight: 600, color: "#1E40AF", textDecoration: "underline" }}>{sol.nombre_paciente}</td>
...
function SolicitudEmergenciaForm({ token, perfil }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      setMsg({ type: "success", text: "Solicitud de emergencia registrada ✓" });
      setNombrePaciente("");
      setEdad("");
      setSubTipo("");
...
function AjustesView({ session }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {/* Solicitud de Emergencia */}
      <div style={cx.card}>
        <p style={{ margin: "0 0 16px", fontSize: 13, fontWeight: 700, color: "#DC2626", textTransform: "uppercase", letterSpacing: "0.06em" }}>🚨 Crear Solicitud de Emergencia</p>
        <SolicitudEmergenciaForm token={token} perfil={perfil} />
      </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F8FAFC" }}>
      ...
      <div style={{ maxWidth: 880, margin: "0 auto", padding: "20px 16px" }}>
        ...
        {view === "censo"        && <CensoView solicitudes={sols} loading={loading} token={token} onSelectSolicitud={setSelected} />}
