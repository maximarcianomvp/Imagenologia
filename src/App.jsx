          ...
          {perfiles.filter(p => p.en_guardia).length > 0 ? (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {perfiles.filter(p => p.en_guardia).map(res => (
                <span key={res.id} style={{ background: "#DC2626", color: "#fff", padding: "6px 12px", borderRadius: 6, fontSize: 13, fontWeight: 600 }}>
                  {res.nombre}
                </span>
              ))}
            </div>
          ) : (
            <p style={{ margin: 0, fontSize: 13, color: "#7F1D1D" }}>Sin guardias asignadas hoy.</p>
          )}
        </div>
      )}
