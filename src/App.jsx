...
function CensoView({ solicitudes, loading, token, onSelectSolicitud, adscritos, perfiles }) {
  try {
    return <div style={{ padding: "20px", fontSize: "16px", background: "#FFF" }}>📊 Censo - En construcción</div>;
  } catch (e) {
    return <div style={{ padding: "20px", color: "red" }}>Error: {e.toString()}</div>;
  }
}
...
function PanelJefe({ session, onLogout }) {
  ...
  useEffect(() => {
    try {
      ...
      console.error("Error loading adscritos:", e);
    }
  }, [session.token]);
    ...
    return (
      <div style={{ minHeight: "100vh", background: "#F8FAFC" }}>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "20px 16px" }}>
          <CensoView solicitudes={sols} loading={loading} token={session.token} onSelectSolicitud={() => {}} adscritos={ads} perfiles={perfs} />
        </div>
      </div>
    );
  } catch (e) {
    return <div style={{ padding: "20px", color: "red", fontSize: "16px" }}>Error en PanelJefe: {e.toString()}</div>;
  }
}
