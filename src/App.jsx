    ...
    <div style={{ padding: "20px" }}>
      <div style={{ marginBottom: "20px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
          <h2 style={{ margin: 0 }}>📊 Censo</h2>
          <input type="date" value={fecha} onChange={e => setFecha(e.target.value)} style={{ padding: "8px", borderRadius: "5px", border: "1px solid #CCC" }} />
        </div>
        
        {/* Mostrar horario de 24 hrs */}
        <div style={{ background: "#EFF6FF", padding: "15px", borderRadius: "8px", border: "2px solid #1E40AF" }}>
          <p style={{ margin: "0 0 8px 0", fontWeight: "bold", fontSize: "14px", color: "#1E3A8A" }}>🕐 Jornada de 24 horas:</p>
          <div style={{ fontSize: "13px", color: "#1E40AF" }}>
            <span style={{ fontWeight: "bold" }}>Desde:</span> 7:00 AM {fecha} 
            <span style={{ margin: "0 10px", fontWeight: "bold" }}>Hasta:</span> 7:00 AM {new Date(new Date(fecha).getTime() + 86400000).toISOString().split('T')[0]}
          </div>
          <p style={{ margin: "8px 0 0 0", fontSize: "12px", color: "#64748B" }}>
            {fecha} → {new Date(new Date(fecha).getTime() + 86400000).toISOString().split('T')[0]}
          </p>
        </div>
      </div>

      {guardias.length > 0 && (
        <div style={{ background: "#E9D5FF", padding: "15px", borderRadius: "8px", marginBottom: "20px" }}>
          <p style={{ margin: "0 0 10px 0", fontWeight: "bold", color: "#6B21A8" }}>🛡️ En Guardia:</p>
          {guardias.map(g => <span key={g.id} style={{ display: "inline-block", background: "#D8B4FE", padding: "4px 8px", borderRadius: "4px", marginRight: "8px", fontSize: "12px" }}>{g.nombre}</span>)}
        </div>
      )}

      {turnos.map(turno => (
        <div key={turno} style={{ marginBottom: "30px" }}>
          <h3 style={{ background: "#1E3A8A", color: "#fff", padding: "10px", borderRadius: "5px", margin: "0 0 10px 0" }}>🕐 {turno}</h3>
          {groupedByTurno[turno].map((group, idx) => (
            <div key={idx} style={{ marginBottom: "20px", borderLeft: "4px solid #1E3A8A", paddingLeft: "15px" }}>
              <p style={{ fontWeight: "bold", margin: "0 0 10px 0", fontSize: "14px" }}>{group.doctor}</p>
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                  <thead>
                    <tr style={{ background: "#F3F4F6", borderBottom: "2px solid #D1D5DB" }}>
                      <th style={{ padding: "8px", textAlign: "left", borderRight: "1px solid #D1D5DB" }}>Paciente</th>
                      <th style={{ padding: "8px", textAlign: "center", borderRight: "1px solid #D1D5DB", width: "50px" }}>Edad</th>
                      <th style={{ padding: "8px", textAlign: "left", borderRight: "1px solid #D1D5DB" }}>Estudio</th>
                      <th style={{ padding: "8px", textAlign: "left", borderRight: "1px solid #D1D5DB" }}>Servicio</th>
                      <th style={{ padding: "8px", textAlign: "center", borderRight: "1px solid #D1D5DB", width: "70px" }}>Prioridad</th>
                      <th style={{ padding: "8px", textAlign: "center", borderRight: "1px solid #D1D5DB", width: "80px" }}>Estado</th>
                      <th style={{ padding: "8px", textAlign: "left" }}>Hallazgos</th>
                    </tr>
                  </thead>
                  <tbody>
                    {group.estudios.map(sol => (
                      <tr key={sol.id} style={{ borderBottom: "1px solid #E5E7EB", background: sol.es_consulta_externa ? "#F0FDF4" : "white" }}>
                        <td style={{ padding: "8px", borderRight: "1px solid #E5E7EB", cursor: "pointer", color: "#1E40AF", textDecoration: "underline" }} onClick={() => onSelectSolicitud(sol)}>
                          {sol.nombre_paciente}
                        </td>
                        <td style={{ padding: "8px", borderRight: "1px solid #E5E7EB", textAlign: "center" }}>{sol.edad || "-"}</td>
                        <td style={{ padding: "8px", borderRight: "1px solid #E5E7EB", fontSize: "12px" }}>{sol.tipo_estudio}</td>
                        <td style={{ padding: "8px", borderRight: "1px solid #E5E7EB", fontSize: "12px" }}>{sol.servicio_solicitante || sol.servicio_otro || "-"}</td>
                        <td style={{ padding: "8px", borderRight: "1px solid #E5E7EB", textAlign: "center", fontSize: "12px", fontWeight: "bold", color: sol.prioridad === "Urgente" ? "#DC2626" : "#374151" }}>
                          {sol.prioridad || "-"}
                        </td>
                        <td style={{ padding: "8px", borderRight: "1px solid #E5E7EB", textAlign: "center" }}>
                          <span style={{ background: estadoColor[sol.estado] || "#E5E7EB", padding: "4px 8px", borderRadius: "4px", fontSize: "11px", fontWeight: "bold" }}>
                            {sol.estado}
                          </span>
                        </td>
                        <td style={{ padding: "8px", fontSize: "12px", color: "#4B5563" }}>{sol.hallazgos || "-"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
// ─── Solicitudes (Consulta Externa + Hospitalizado) ────────────────────────
function SolicitudesView({ token, perfil, adscritos }) {
  const [activeForm, setActiveForm] = useState("consulta");

  return (
    <div style={{ padding: "20px" }}>
      <h2 style={{ marginTop: 0 }}>➕ Agregar Solicitud</h2>
      ...
      <div style={{ display: "flex", gap: "10px", marginBottom: "20px", borderBottom: "2px solid #E5E7EB", paddingBottom: "10px" }}>
        <button 
          ...
          style={{
            padding: "10px 20px",
            background: activeForm === "consulta" ? "#1E40AF" : "transparent",
            color: activeForm === "consulta" ? "white" : "#1E40AF",
        ...
        <button 
          onClick={() => setActiveForm("hospitalizado")}
          style={{
            padding: "10px 20px",
            background: activeForm === "hospitalizado" ? "#DC2626" : "transparent",
            color: activeForm === "hospitalizado" ? "white" : "#DC2626",
      ...
      {activeForm === "consulta" && <ConsultaExternaForm token={token} perfil={perfil} />}
      {activeForm === "hospitalizado" && <SolicitudEmergenciaForm token={token} perfil={perfil} adscritos={adscritos} />}
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F8FAFC" }}>
      ...
      <div style={{ maxWidth: 880, margin: "0 auto", padding: "20px 16px" }}>
        ...
        {view === "solicitudes"  && <SolicitudesView token={token} perfil={perfil} adscritos={ads} />}
