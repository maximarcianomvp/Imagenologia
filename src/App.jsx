...
function CensoView({ solicitudes, loading, token, onSelectSolicitud, adscritos, perfiles }) {
  ...
  // Guardias con estado actualizado
  const guardias = Array.isArray(perfs) ? perfs.filter(p => p.en_guardia) : [];
  ...
  function exportToExcel() {
    try {
      ...
      if (!window.XLSX) {
        ...
        script.onerror = () => {
          // Si falla CDN, usar alternativa simple
          exportToCSV();
        };
      ...
      // === ENCABEZADO ===
      wsData.push(["CENSO DE IMAGENOLOGÍA"]);
      wsData.push([`Jornada: ${fecha} 7:00 AM → ${new Date(new Date(fecha).getTime() + 86400000).toISOString().split('T')[0]} 7:00 AM`]);
      wsData.push([]);

      // === GUARDIAS ===
      if (guardias.length > 0) {
        wsData.push(["🛡️ RESIDENTES EN GUARDIA"]);
        guardias.forEach(g => wsData.push([g.nombre]));
        wsData.push([]);
      }
      ...
      turnosPresentes.forEach(turno => {
        wsData.push([`TURNO: ${turno}`, "", "", "", "", "", "", ""]);
        wsData.push(["Adscrito", "Paciente", "Edad", "Estudio", "Servicio", "Prioridad", "Estado", "Hallazgos"]);
        
        groupedByTurno[turno].forEach(group => {
          group.estudios.forEach((sol, idx) => {
            wsData.push([
              idx === 0 ? group.doctor : "",
              sol.nombre_paciente,
              sol.edad || "-",
      ...
      for (let R = range.s.r; R <= range.e.r; ++R) {
        for (let C = range.s.c; C <= range.e.c; ++C) {
          ...
          if (R === 0) {
            cell.font = { bold: true, size: 16, color: { rgb: "FF1E40AF" } };
            cell.fill = { fgColor: { rgb: "FFDBEAFE" } };
            cell.alignment = { horizontal: "center", vertical: "center", wrapText: true };
            ws["!rows"][R] = { hpt: 30 };
          }
          // Subtítulo jornada
          else if (R === 1) {
            cell.font = { size: 11, color: { rgb: "FF0F172A" } };
            cell.fill = { fgColor: { rgb: "FFF0F9FF" } };
            cell.alignment = { horizontal: "center", vertical: "center", wrapText: true };
            ws["!rows"][R] = { hpt: 22 };
          }
          // Encabezado de guardias
          else if (cell.v && typeof cell.v === "string" && cell.v.startsWith("🛡️")) {
            cell.font = { bold: true, size: 12, color: { rgb: "FF6B21A8" } };
            cell.fill = { fgColor: { rgb: "FFF3E8FF" } };
            ws["!rows"][R] = { hpt: 20 };
          }
          // Encabezado de turno
          else if (cell.v && typeof cell.v === "string" && cell.v.startsWith("TURNO:")) {
            cell.font = { bold: true, size: 11, color: { rgb: "FFFFFFFF" } };
            cell.fill = { fgColor: { rgb: "FF1E3A8A" } };
            cell.alignment = { horizontal: "left", vertical: "center", wrapText: true };
            ws["!rows"][R] = { hpt: 22 };
          }
          // Header de tabla (Adscrito, Paciente, etc)
          else if (R > 0 && wsData[R] && wsData[R][0] === cell.v && 
                   ["Adscrito", "Paciente", "Edad", "Estudio", "Servicio", "Prioridad", "Estado", "Hallazgos"].includes(cell.v)) {
            cell.font = { bold: true, size: 11, color: { rgb: "FFFFFFFF" } };
            cell.fill = { fgColor: { rgb: "FF374151" } };
            cell.alignment = { horizontal: "center", vertical: "center", wrapText: true };
            ws["!rows"][R] = { hpt: 22 };
          }
          // Datos normales
          else {
            cell.font = { size: 10, color: { rgb: "FF1F2937" } };
            // Alternar colores de fila
            if (R % 2 === 0 && !cell.v?.toString().startsWith("TURNO:")) {
              cell.fill = { fgColor: { rgb: "FFF9FAFB" } };
            }
            cell.alignment = { horizontal: "left", vertical: "center", wrapText: true };
            ws["!rows"][R] = { hpt: 20 };
          }
  ...
  function exportToCSV() {
    try {
      ...
      // Título
      csvData.push("CENSO DE IMAGENOLOGÍA");
      csvData.push(`Jornada: ${fecha} 7:00 AM → ${new Date(new Date(fecha).getTime() + 86400000).toISOString().split('T')[0]} 7:00 AM`);
      csvData.push("");

      // Guardias
      if (guardias.length > 0) {
        csvData.push("🛡️ RESIDENTES EN GUARDIA");
        guardias.forEach(g => csvData.push(g.nombre));
        csvData.push("");
      }
      ...
      turnosPresentes.forEach(turno => {
        csvData.push(`TURNO: ${turno}`);
        csvData.push(["Adscrito", "Paciente", "Edad", "Estudio", "Servicio", "Prioridad", "Estado", "Hallazgos"].join(","));
      ...
      link.download = `Censo_${fecha}.csv`;
      link.click();
    } catch (err) {
  ...
  return (
    <div style={{ padding: "20px", background: "linear-gradient(135deg, #F0F9FF 0%, #F8FAFC 100%)", minHeight: "100vh" }}>
      ...
      {/* Guardias */}
      {guardias.length > 0 && (
        <div style={{ background: "linear-gradient(135deg, #F3E8FF 0%, #FAF5FF 100%)", padding: "16px 20px", borderRadius: "12px", marginBottom: "25px", border: "2px solid #E9D5FF", boxShadow: "0 2px 4px rgba(168, 85, 247, 0.1)" }}>
          <p style={{ margin: "0 0 12px 0", fontWeight: "700", color: "#6B21A8", fontSize: "14px", textTransform: "uppercase", letterSpacing: "0.5px" }}>🛡️ Residentes en Guardia</p>
