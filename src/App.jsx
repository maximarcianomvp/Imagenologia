...
function CensoView({ solicitudes, loading }) {
  ...
  // Agrupar por adscrito, ordenado por hora dentro de cada grupo
  const grouped = {};
  [...list]
  ...
  // Ordenar grupos: primero los que tienen adscrito, al final "Sin adscrito"
  ...
  function exportExcel() {
    ...
    rows.push({
      ...
      "Horario": "",
      "Estado": "",
      "Hora solicitud": fecha,
    });
    rows.push({});

    groupKeys.forEach(doctor => {
      // Fila de doctor
      rows.push({
        "Doctor": doctor.toUpperCase(),
        "Paciente": `(${grouped[doctor].length} estudio${grouped[doctor].length !== 1 ? "s" : ""})`,
        "Estudio": "", "Modalidad": "", "Servicio": "",
        "Prioridad": "", "Horario": "", "Estado": "", "Hora solicitud": "",
      });
      ...
      grouped[doctor].forEach((sol, i) => {
        ...
        rows.push({
          "Doctor": "",
          "Paciente": sol.nombre_paciente,
          "Estudio": sol.tipo_estudio,
          "Modalidad": sol.sub_tipo || "—",
          "Servicio": svc,
          "Prioridad": sol.prioridad,
          "Horario": sol.horario_programado || "—",
          "Estado": ESTADOS_CFG[sol.estado]?.label || sol.estado,
          "Hora solicitud": new Date(sol.created_at).toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" }),
        });
      });

      rows.push({}); // Espacio entre doctores
    });

    // Totales
    rows.push({
      "Doctor": "TOTALES",
      "Paciente": `Total: ${cnt.total}`,
      "Estudio": `Realizados: ${cnt.realizado}`,
      "Modalidad": `En proceso: ${cnt.en_proceso}`,
      "Servicio": `Pendientes: ${cnt.pendiente}`,
      "Prioridad": "", "Horario": "", "Estado": "", "Hora solicitud": "",
    });

    const ws = XLSX.utils.json_to_sheet(rows);
    ws["!cols"] = [20, 28, 18, 16, 20, 12, 14, 14, 16].map(w => ({ wch: w }));
    XLSX.utils.book_append_sheet(wb, ws, "Censo");
    XLSX.writeFile(wb, `Censo_Imagenologia_${fecha}.xlsx`);
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <h2 style={{ margin: 0, fontSize: 17, fontWeight: 700 }}>
          Censo — {new Date().toLocaleDateString("es-MX", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
        </h2>
        <button
          onClick={exportExcel}
          disabled={list.length === 0}
          style={{ background: "#065F46", color: "#fff", border: "none", borderRadius: 8, padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: list.length === 0 ? "not-allowed" : "pointer", opacity: list.length === 0 ? 0.5 : 1, fontFamily: "inherit" }}
        >
          📥 Exportar Excel
        </button>
      </div>
      ...
      : list.length === 0 ? <p style={{ textAlign: "center", color: "#9CA3AF", padding: 32 }}>Sin estudios registrados hoy.</p>
      : (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {groupKeys.map(doctor => (
            <div key={doctor} style={{ ...cx.card, padding: 0, overflow: "hidden" }}>
              {/* Doctor header */}
              <div style={{ background: doctor === "Sin adscrito asignado" ? "#FEF3C7" : "#EFF6FF", padding: "10px 16px", borderBottom: "1px solid #E5E7EB", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontWeight: 700, fontSize: 14, color: doctor === "Sin adscrito asignado" ? "#92400E" : "#1E40AF" }}>
                  {doctor === "Sin adscrito asignado" ? "⚠️ " : "👨‍⚕️ "}{doctor}
                </span>
                <span style={{ fontSize: 12, color: "#6B7280" }}>{grouped[doctor].length} estudio{grouped[doctor].length !== 1 ? "s" : ""}</span>
              </div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                <thead>
                  <tr style={{ background: "#F9FAFB" }}>
                    {["Hora", "Paciente", "Estudio", "Servicio", "Prioridad", "Horario", "Estado"].map(h => (
                      <th key={h} style={{ padding: "8px 12px", textAlign: "left", fontSize: 11, color: "#6B7280", fontWeight: 700, letterSpacing: "0.04em", textTransform: "uppercase", borderBottom: "1px solid #E5E7EB" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {grouped[doctor].map((sol) => {
                    const E   = ESTADOS_CFG[sol.estado] || ESTADOS_CFG.pendiente;
                    const P   = PRIORIDADES_CFG[sol.prioridad] || PRIORIDADES_CFG.Rutina;
                    const svc = sol.servicio_solicitante === "Otro" ? sol.servicio_otro : sol.servicio_solicitante;
                    return (
                      <tr key={sol.id} style={{ borderBottom: "1px solid #F3F4F6" }}>
                        <td style={{ padding: "9px 12px", color: "#6B7280", fontWeight: 500, whiteSpace: "nowrap" }}>
                        ...
                        <td style={{ padding: "9px 12px" }}><Badge label={sol.prioridad} bg={P.bg} color={P.color} /></td>
