...
function CensoView({ solicitudes, loading, token, onSelectSolicitud, adscritos, perfiles }) {
  ...
  // Auto-refresh perfiles cada 10 segundos para guardias
  useEffect(() => {
    if (!token) return;
  ...
  // Cargar guardias historial cuando cambie la fecha
  useEffect(() => {
  ...
  // Filtrar solicitudes por jornada 7am-7am
  ...
  // Guardias: usar historial si existe, sino usar las actuales en vivo
  const guardias = guardiasHistorial?.residentes || (Array.isArray(perfs) ? perfs.filter(p => p.en_guardia) : []);

  const estadoColor = {
    "pendiente": "#FBBF24", "agendado": "#A7F3D0", "en_proceso": "#93C5FD",
    "reagendado": "#86EFAC", "postpuesto": "#D8B4FE", "realizado": "#34D399",
    "denegado": "#FCA5A5", "expirado": "#D1D5DB", "cancelado": "#F87171"
  };

  function exportToExcel() {
    try {
      const turnoOrder = ["Matutino", "Vespertino", "Nocturno", "Dia jornada acumulada", "Jornada acumulada", "Festivo dia", "Festivo noche"];
      const turnosPresentes = turnoOrder.filter(t => groupedByTurno[t]);

      // Crear HTML con estilos de Excel
      let html = `
        <html>
        <head>
          <meta charset="utf-8">
          <style>
            ...
            .header { 
              ...
              margin-bottom: 5px; border-bottom: 3px solid #1E40AF; 
              padding-bottom: 10px;
            }
            .jornada { 
              font-size: 12px; color: #0F172A; background: #EFF6FF; 
              padding: 10px; border-radius: 5px; margin-bottom: 20px; 
              border-left: 4px solid #1E40AF;
            }
            .guardias { 
              background: #F3E8FF; padding: 10px; 
              margin-bottom: 20px; border-radius: 5px; 
              border-left: 4px solid #7C3AED;
            }
            .guardias p { margin: 0; font-weight: bold; color: #6B21A8; font-size: 12px; }
            .turno-section { page-break-inside: avoid; margin-bottom: 30px; }
            .turno-header { 
              background: #1E3A8A; color: white; 
              padding: 12px 15px; font-size: 14px; font-weight: bold; 
              border-radius: 5px 5px 0 0; margin-bottom: 0;
            }
            .adscrito-group { border: 1px solid #D1D5DB; margin-bottom: 0; }
            .adscrito-name { 
            ...
            th { 
              ...
              font-weight: bold; border-bottom: 2px solid #1F2937;
            }
            td { 
            ...
            .patient { color: #1E40AF; font-weight: 600; }
            .estado { padding: 3px 6px; border-radius: 3px; font-size: 9px; font-weight: bold; }
            .estado-pendiente { background: #FBBF24; color: #78350F; }
            .estado-realizado { background: #34D399; color: #065F46; }
            .estado-en_proceso { background: #93C5FD; color: #1E3A8A; }
            .urgencia { color: #DC2626; font-weight: bold; }
            .footer { margin-top: 30px; font-size: 10px; color: #6B7280; border-top: 1px solid #E5E7EB; padding-top: 10px; }
          </style>
        </head>
        <body>
          <div class="header">📊 CENSO DE IMAGENOLOGÍA</div>
          <div class="jornada">
            <strong>Jornada:</strong> ${fecha} 7:00 AM → ${new Date(new Date(fecha).getTime() + 86400000).toISOString().split('T')[0]} 7:00 AM
          </div>
      `;
      ...
      if (guardias.length > 0) {
        html += `
          <div class="guardias">
            <p>🛡️ RESIDENTES EN GUARDIA</p>
            <div style="margin-top: 5px; font-size: 11px;">
              ${guardias.map(g => `<span style="display: inline-block; background: #C4B5FD; color: #5B21B6; padding: 4px 8px; border-radius: 12px; margin-right: 8px;">${g.nombre}</span>`).join("")}
            </div>
          </div>
        `;
      }
      ...
      turnosPresentes.forEach(turno => {
        const total = groupedByTurno[turno].reduce((sum, g) => sum + g.estudios.length, 0);
        
        html += `
          <div class="turno-section">
            <div class="turno-header">🕐 ${turno} (${total} estudios)</div>
            <div class="adscrito-group">
        `;

        groupedByTurno[turno].forEach((group, idx) => {
          html += `
            <div class="adscrito-name">👨‍⚕️ ${group.doctor}</div>
            <table>
              <thead>
                <tr>
                  <th>Paciente</th>
          ...
          group.estudios.forEach(sol => {
            const estadoClass = `estado estado-${sol.estado}`;
            const urgencia = sol.prioridad === "Urgencia" || sol.prioridad === "Urgente" ? `<span class="urgencia">${sol.prioridad}</span>` : (sol.prioridad || "-");
            
            html += `
              <tr>
                <td class="patient">${sol.nombre_paciente}</td>
                <td>${sol.edad || "-"}</td>
                <td>${sol.tipo_estudio}</td>
                <td>${sol.servicio_solicitante || sol.servicio_otro || "-"}</td>
                <td>${urgencia}</td>
                <td><span class="${estadoClass}">${sol.estado}</span></td>
                <td>${sol.hallazgos ? sol.hallazgos.substring(0, 40) + (sol.hallazgos.length > 40 ? "..." : "") : "-"}</td>
              </tr>
            `;
          });
      ...
      html += `
        <div class="footer">
          Generado automáticamente por Imagenología • ${new Date().toLocaleString("es-MX")}
        </div>
      </body>
      </html>
      `;
      ...
      link.setAttribute("download", `Censo_${fecha}.xls`);
  ...
  return (
    <div style={{ padding: "20px", background: "linear-gradient(135deg, #F0F9FF 0%, #F8FAFC 100%)", minHeight: "100vh" }}>
      {/* Header */}
      <div style={{ marginBottom: "25px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
          <h1 style={{ margin: 0, fontSize: "32px", fontWeight: "800", color: "#0F172A", display: "flex", alignItems: "center", gap: "12px" }}>
            <span style={{ fontSize: "40px" }}>📊</span>
            Censo de Imagenología
          </h1>
        ...
        {/* Jornada */}
        <div style={{ background: "linear-gradient(135deg, #1E40AF 0%, #1E3A8A 100%)", color: "white", padding: "18px", borderRadius: "12px", boxShadow: "0 4px 6px rgba(30, 64, 175, 0.1)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", gap: "20px", alignItems: "center" }}>
            <div>
              <p style={{ margin: "0 0 6px 0", fontSize: "12px", fontWeight: "600", opacity: "0.9", textTransform: "uppercase", letterSpacing: "0.5px" }}>🕐 Inicio de jornada</p>
              <p style={{ margin: 0, fontSize: "18px", fontWeight: "700" }}>7:00 AM • {fecha}</p>
            </div>
            <div style={{ fontSize: "24px", opacity: "0.6" }}>→</div>
            <div style={{ textAlign: "right" }}>
              <p style={{ margin: "0 0 6px 0", fontSize: "12px", fontWeight: "600", opacity: "0.9", textTransform: "uppercase", letterSpacing: "0.5px" }}>🕐 Fin de jornada</p>
              <p style={{ margin: 0, fontSize: "18px", fontWeight: "700" }}>7:00 AM • {new Date(new Date(fecha).getTime() + 86400000).toISOString().split('T')[0]}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Guardias */}
      {guardias.length > 0 && (
        <div style={{ background: "linear-gradient(135deg, #F3E8FF 0%, #FAF5FF 100%)", padding: "16px 20px", borderRadius: "12px", marginBottom: "25px", border: "2px solid #E9D5FF", boxShadow: "0 2px 4px rgba(168, 85, 247, 0.1)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <p style={{ margin: "0 0 12px 0", fontWeight: "700", color: "#6B21A8", fontSize: "14px", textTransform: "uppercase", letterSpacing: "0.5px" }}>
              🛡️ Residentes en Guardia {guardiasHistorial ? "📋 (Registrado)" : "🔴 (En vivo)"}
            </p>
      ...
      {/* Turnos */}
      {turnos.map(turno => (
        <div key={turno} style={{ marginBottom: "28px" }}>
          {/* Turno Header */}
          <div style={{ 
            ...
            <span style={{ background: "rgba(255,255,255,0.2)", padding: "4px 12px", borderRadius: "20px", fontSize: "12px", fontWeight: "600" }}>
              {groupedByTurno[turno].reduce((sum, g) => sum + g.estudios.length, 0)} estudios
            </span>
          </div>

          {/* Adscritos en turno */}
          {groupedByTurno[turno].map((group, idx) => (
            <div key={idx} style={{ background: "white", borderLeft: "5px solid #1E40AF", marginTop: idx === 0 ? 0 : "16px" }}>
              <div style={{ padding: "12px 20px", background: "#F8FAFC", borderBottom: "1px solid #E5E7EB" }}>
                <h4 style={{ margin: 0, fontSize: "15px", fontWeight: "700", color: "#0F172A" }}>👨‍⚕️ {group.doctor}</h4>
              </div>
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                  <thead>
                    <tr style={{ background: "#F3F4F6", borderBottom: "2px solid #D1D5DB" }}>
                      <th style={{ padding: "12px 16px", textAlign: "left", borderRight: "1px solid #E5E7EB", fontWeight: "700", color: "#374151" }}>Paciente</th>
                      <th style={{ padding: "12px 8px", textAlign: "center", borderRight: "1px solid #E5E7EB", fontWeight: "700", color: "#374151", width: "50px" }}>Edad</th>
                      <th style={{ padding: "12px 16px", textAlign: "left", borderRight: "1px solid #E5E7EB", fontWeight: "700", color: "#374151" }}>Estudio</th>
                      <th style={{ padding: "12px 16px", textAlign: "left", borderRight: "1px solid #E5E7EB", fontWeight: "700", color: "#374151" }}>Servicio</th>
                      <th style={{ padding: "12px 8px", textAlign: "center", borderRight: "1px solid #E5E7EB", fontWeight: "700", color: "#374151", width: "70px" }}>Prioridad</th>
                      <th style={{ padding: "12px 8px", textAlign: "center", borderRight: "1px solid #E5E7EB", fontWeight: "700", color: "#374151", width: "85px" }}>Estado</th>
                      <th style={{ padding: "12px 16px", textAlign: "left", fontWeight: "700", color: "#374151" }}>Hallazgos</th>
                    </tr>
                  </thead>
                  <tbody>
                    {group.estudios.map((sol, rowIdx) => (
                      <tr 
                        key={sol.id} 
                        style={{ 
                        ...
                        onMouseOver={e => e.currentTarget.style.background = sol.es_consulta_externa ? "#F0FDF4" : "#F0F7FF"}
                        onMouseOut={e => e.currentTarget.style.background = rowIdx % 2 === 0 ? "white" : "#F9FAFB"}
                      >
                        <td 
                          style={{ 
                            padding: "12px 16px", 
                        ...
                        <td style={{ padding: "12px 8px", borderRight: "1px solid #E5E7EB", textAlign: "center", color: "#6B7280" }}>{sol.edad || "-"}</td>
                        <td style={{ padding: "12px 16px", borderRight: "1px solid #E5E7EB", fontSize: "12px", color: "#374151" }}>{sol.tipo_estudio}</td>
                        <td style={{ padding: "12px 16px", borderRight: "1px solid #E5E7EB", fontSize: "12px", color: "#374151" }}>{sol.servicio_solicitante || sol.servicio_otro || "-"}</td>
                        <td style={{ padding: "12px 8px", borderRight: "1px solid #E5E7EB", textAlign: "center", fontSize: "12px", fontWeight: "700", color: sol.prioridad === "Urgente" ? "#DC2626" : sol.prioridad === "Urgencia" ? "#EA580C" : "#374151" }}>
                          {sol.prioridad === "Urgencia" ? "🔴 Urg" : sol.prioridad === "Urgente" ? "🔴 URG" : sol.prioridad || "-"}
                        </td>
                        <td style={{ padding: "12px 8px", borderRight: "1px solid #E5E7EB", textAlign: "center" }}>
                          <span style={{ background: estadoColor[sol.estado] || "#E5E7EB", color: sol.estado === "realizado" ? "#065F46" : sol.estado === "pendiente" ? "#78350F" : "#374151", padding: "5px 10px", borderRadius: "6px", fontSize: "11px", fontWeight: "700" }}>
                            {sol.estado}
                          </span>
                        </td>
                        <td style={{ padding: "12px 16px", fontSize: "12px", color: "#4B5563" }}>{sol.hallazgos ? `${sol.hallazgos.substring(0, 30)}${sol.hallazgos.length > 30 ? "..." : ""}` : "-"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
...
function SolicitudEmergenciaForm({ token, perfil, adscritos }) {
...
function ConsultaExternaForm({ token, perfil, adscritos }) {
  const [nombrePaciente, setNombrePaciente] = useState("");
  const [edad, setEdad] = useState("");
        ...
        {view === "solicitudes" && (
          <>
            ...
            <div style={{ ...cx.card, marginTop: 20 }}>
              ...
              <SolicitudEmergenciaForm token={token} perfil={perfil} adscritos={ads} />
            </div>
            ...
            <div style={{ ...cx.card, marginTop: 20 }}>
              ...
              <ConsultaExternaForm token={token} perfil={perfil} adscritos={ads} />
            </div>
          </>
        )}
