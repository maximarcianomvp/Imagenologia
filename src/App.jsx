...
// Rango de jornada (7am a 7am siguiente)
function getJornadaRange(dateStr) {
  const [year, month, day] = dateStr.split("-").map(Number);
  const start = new Date(year, month - 1, day, 7, 0, 0);
  const end = new Date(year, month - 1, day + 1, 7, 0, 0);
  
  const startISO = start.toISOString().split("T")[0];
  const startTime = "T07:00:00";
  const endISO = end.toISOString().split("T")[0];
  const endTime = "T07:00:00";
...
function CensoView({ solicitudes, loading }) {
  ...
  useEffect(() => {
    // Nota: Esta función necesita token, pero lo obtenemos del contexto global
    // Por ahora mostramos guardias de perfiles que cargamos
  }, []);
  const { startISO, startTime, endISO, endTime } = getJornadaRange(fecha);
  const list = solicitudes.filter(s => {
    const sDate = s.created_at?.split("T")[0];
    const sTime = s.created_at?.split("T")[1];
    if (!sDate || !sTime) return false;
    
    // Si la fecha es igual a startISO, solo incluir si la hora es >= 7am
    if (sDate === startISO) return sTime >= startTime;
    // Si la fecha es igual a endISO, solo incluir si la hora es < 7am
    if (sDate === endISO) return sTime < endTime;
    // Ningún otro día
    return false;
  });
  ...
  function exportExcel() {
    ...
    turnos.forEach((turno) => {
      html += `
        <tr>
          <td colspan="8" style="background:#1E3A8A;color:white;font-weight:bold;font-size:13px;padding:11px 16px;">
      ...
      groupedByTurno[turno]?.forEach((group, idx) => {
        ...
        html += `
          <tr>
            <td colspan="8" style="background:${clr.header};color:white;font-weight:bold;font-size:12px;padding:9px 14px;">
          ...
          html += `
            <tr style="background:${rowBg};">
              <td style="padding:8px 12px;border:1px solid ${clr.border};color:${clr.text};font-weight:bold;">
                ${toMexicoTime(sol.created_at).toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })}
              </td>
        ...
        html += `<tr><td colspan="9" style="padding:5px;"></td></tr>`;
      });
    });
  ...
  return (
    <div>
      ...
      {/* Guardias */}
      {loading ? null : (
        <div style={{ background: "#FEE2E2", borderRadius: 12, border: "1.5px solid #FECACA", padding: "16px", marginBottom: 24 }}>
          <p style={{ margin: "0 0 12px", fontSize: 13, fontWeight: 700, color: "#991B1B", textTransform: "uppercase", letterSpacing: "0.05em" }}>🚨 Residentes de guardia</p>
          {/* Los guardias se cargarían aquí desde perfiles con en_guardia = true */}
          <p style={{ margin: 0, fontSize: 13, color: "#7F1D1D" }}>Esta sección muestra quiénes están de guardia hoy (se actualiza en tiempo real desde Ajustes).</p>
        </div>
      )}
                    ...
                    <div key={group.doctor} style={{ borderRadius: 12, overflow: "hidden", border: `1.5px solid ${clr.border}`, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
                      ...
                      <div style={{ overflowX: "auto" }}>
                        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, minWidth: 700 }}>
                          <thead>
                            <tr style={{ background: clr.light }}>
                              {["Hora", "Paciente", "Estudio", "Servicio", "Prioridad", "Horario", "Estado", "Hallazgos"].map(h => (
                            ...
                            {group.estudios.map((sol, i) => {
                              ...
                              return (
                                <tr key={sol.id} style={{ background: i % 2 === 0 ? "#fff" : clr.light, borderBottom: `1px solid ${clr.border}66` }}>
                                  <td style={{ padding: "8px 12px", color: clr.text, fontWeight: 700, whiteSpace: "nowrap" }}>
                                    {toMexicoTime(sol.created_at).toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })}
                                  </td>
                                  ...
                                  <td style={{ padding: "8px 12px", color: "#374151" }}>{sol.horario_programado || <span style={{ color: "#D1D5DB" }}>—</span>}</td>
...
function AjustesView({ session }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={cx.card}>
        <p style={{ margin: "0 0 16px", fontSize: 13, fontWeight: 700, color: "#DC2626", textTransform: "uppercase", letterSpacing: "0.06em" }}>🚨 Residentes de guardia hoy</p>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {perfiles.filter(p => ["r1", "r3", "r4", "admin"].includes(p.rol)).map(res => (
            <div key={res.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", borderRadius: 8, background: res.en_guardia ? "#FEE2E2" : "#F9FAFB", border: `1px solid ${res.en_guardia ? "#FECACA" : "#E5E7EB"}` }}>
              <span style={{ fontSize: 14, fontWeight: 500, color: res.en_guardia ? "#991B1B" : "#374151" }}>{res.nombre}</span>
              <button
                onClick={() => {
                  setToggling(res.id);
                ...
                style={{ padding: "5px 16px", borderRadius: 20, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600, fontFamily: "inherit", background: res.en_guardia ? "#DC2626" : "#D1D5DB", color: res.en_guardia ? "#fff" : "#374151", opacity: toggling === res.id ? 0.5 : 1 }}
              >
...
function PanelResidente({ session, onLogout }) {
  ...
  const { startISO: todayStart, startTime: todayStartTime, endISO: todayEnd, endTime: todayEndTime } = getJornadaRange(hoy);
  const todaySols = sols.filter(s => {
    const sDate = s.created_at?.split("T")[0];
    const sTime = s.created_at?.split("T")[1];
    if (!sDate || !sTime) return false;
    if (sDate === todayStart) return sTime >= todayStartTime;
    if (sDate === todayEnd) return sTime < todayEndTime;
    return false;
  });
  ...
  const filtered = (() => {
    ...
    if (area && area !== "Todas") {
      ...
      f = f.filter(s => estudiosArea.some(tipo => s.tipo_estudio?.includes(tipo)));
    }
    return f;
  })();
