...
function CensoView({ solicitudes, loading, token, onSelectSolicitud, adscritos, perfiles }) {
  ...
  [...list]
    ...
    .forEach(sol => {
      ...
      const doctor = adscritoObj?.nombre || "Sin adscrito asignado";
  ...
  function exportExcel() {
    try {
      ...
      .sort((a, b) => new Date(a.created_at) - new Date(b.created_at))
      .forEach(sol => {
        const adscritoObj = adscritos.find(a => a.id === sol.adscrito_id);
        const turno = adscritoObj?.turno_actual || "Sin asignar";
        const doctor = adscritoObj?.nombre || "Sin adscrito asignado";
        const key = `${turno}|${doctor}`;
        if (!grouped[key]) grouped[key] = { turno, doctor, estudios: [] };
  ...
  return (
    ...
    console.error("Error en CensoView:", err);
    return <div style={{ padding: 20, color: "red" }}>Error en Censo: {err.message}</div>;
  }
}
...
function PanelJefe({ session, onLogout }) {
  ...
  useEffect(() => {
    db.getAdscritos(session.token).then(d => setAds(Array.isArray(d) ? d.filter(a => a.activo) : []));
  }, [session.token]);

  useEffect(() => {
    db.getPerfiles(session.token).then(d => setPerfs(Array.isArray(d) ? d : []));
  }, [session.token]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#F8FAFC" }}>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "20px 16px" }}>
        <CensoView solicitudes={sols} loading={loading} adscritos={ads} perfiles={perfs} />
      </div>
    </div>
  );
}
