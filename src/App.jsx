...
function CensoView({ solicitudes, loading, token, onSelectSolicitud, adscritos, perfiles }) {
  ...
  function exportToExcel() {
    try {
      ...
      // Título y jornada
      worksheetData.push(["CENSO DE IMAGENOLOGÍA"]);
      worksheetData.push([`Jornada: ${fecha} 7:00 AM → ${new Date(new Date(fecha).getTime() + 86400000).toISOString().split('T')[0]} 7:00 AM`]);
      worksheetData.push([]);
      
      // Guardias
      if (guardias.length > 0) {
        worksheetData.push(["RESIDENTES EN GUARDIA"]);
        guardias.forEach(g => worksheetData.push([g.nombre]));
        worksheetData.push([]);
      }

      // Datos por turno
      turnos.forEach(turno => {
        worksheetData.push([`TURNO: ${turno}`]);
        worksheetData.push(["Adscrito", "Paciente", "Edad", "Estudio", "Servicio", "Prioridad", "Estado", "Hallazgos"]);
      ...
      // Usar librería SheetJS si está disponible
      if (typeof window !== "undefined" && window.XLSX) {
        const ws = window.XLSX.utils.aoa_to_sheet(worksheetData);
        const wb = window.XLSX.utils.book_new();
        window.XLSX.utils.book_append_sheet(wb, ws, "Censo");
        window.XLSX.writeFile(wb, `Censo_${fecha}.xlsx`);
      } else {
        // Fallback: crear CSV
        const csv = worksheetData.map(row => row.map(cell => `"${cell}"`).join(",")).join("\n");
