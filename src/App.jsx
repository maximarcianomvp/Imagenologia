...
function SolicitudForm() {
  ...
  const qrParam    = params.get("qr"); // Cambio: buscar "qr" en lugar de "token"
  ...
  const [qrTokenData, setQrTokenData] = useState(null); // Guardar datos del QR

  useEffect(() => {
    if (!qrParam) { setTokenOk(false); return; }
    // Validar que el QR existe en tokens_qr
    db.validateToken(qrParam).then(t => {
      setTokenOk(!!t);
      if (t) setQrTokenData(t);
    });
  }, [qrParam]);
  ...
  async function submit(e) {
    ...
    setBusy(true);
    try {
      const valid = await db.validateToken(qrParam);
      if (!valid) {
      ...
      const formCleaned = {
        ...
        token_qr: qrParam, // Guardar el token QR usado
        token_qr_id: qrTokenData?.id, // Guardar el ID del registro de tokens_qr
      };
      await db.createSolicitud(formCleaned);
      // No marcar como usado, solo guardar que fue usado en la solicitud
      setDone(true);
    } catch (err) {
