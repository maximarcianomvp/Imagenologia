...
function QRScreen() {
  ...
  const url = `${window.location.origin}${window.location.pathname}?page=form`;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "linear-gradient(160deg, #EFF6FF 0%, #DBEAFE 100%)", padding: 32, gap: 28 }}>
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ margin: "10px 0 0", fontSize: 16, color: "#374151" }}>Escanea el código con tu teléfono para enviar tu solicitud</p>
      </div>
...
function SolicitudForm() {
  ...
  async function submit(e) {
    e.preventDefault();
    setBusy(true);
    try { await db.createSolicitud(form); setDone(true); }
